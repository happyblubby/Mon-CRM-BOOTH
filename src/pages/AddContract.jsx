...
export default function AddContract() {
  ...
  useEffect(() => {
    const loadPrerequisites = async () => {
      try {
        ...
        toast.error("Failed to load clients and invoices.");
      }
    };
    loadPrerequisites();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Contract.create(dataToSubmit);
      toast.success("Contract created successfully!");
      navigate(createPageUrl("CRM", { tab: "contracts" }));
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        >
          <Button
            variant="outline"
            size="icon"
            onClick={() => navigate(createPageUrl("CRM", { tab: "contracts" }))}
          ...
          <div>
            ...
            <p className="text-gray-600 text-lg mt-2">Log a new signed contract</p>
          </div>
        </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select onValueChange={handleInvoiceChange}>
                            ...
                            <SelectContent>{invoices.map(i => <SelectItem key={i.id} value={i.id}>{i.invoice_number} - {i.client_name}</SelectItem>)}</SelectContent>
                        </Select>
                    </div>
                    <div className="space-y-2">
                ...
                <div className="flex justify-end gap-4 pt-6 border-t border-gray-100">
                  ...
                  <Button type="submit" disabled={isSubmitting} className="bg-gradient-to-r from-purple-500 to-purple-600 text-white px-8 py-3 rounded-2xl">
                    {isSubmitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Saving...</> : <><Save className="w-5 h-5 mr-2" /> Save Contract</>}
                  </Button>
                </div>
              </form>
            </CardContent>
